...
function reconcileActiveJob(workspaceRoot, job, options = {}) {
  ...
  if (!Number.isFinite(pid) || pid <= 0) {
    staleReason = "no worker PID is recorded";
  } else if (!processAlive(pid)) {
    staleReason = `worker process ${pid} is no longer running`;
  }
  ...
  const patch = {
    ...
    pid: null,
    completedAt,
    errorMessage: `Claude companion job marked stale: ${staleReason}.`,
    stale: true
  };
...
function listReconciledJobs(workspaceRoot, options = {}) {
  return listJobs(workspaceRoot).map((job) => reconcileActiveJob(workspaceRoot, job, options));
}

function getJobTypeLabel(job) {
  if (typeof job.kindLabel === "string" && job.kindLabel) {
    const isTaskJob = job.kind === "task" || job.jobClass === "task";
    if (!isTaskJob || (job.kindLabel !== "rescue" && job.kindLabel !== "task")) {
...
export function resolveResultJob(cwd, reference) {
  const workspaceRoot = resolveWorkspaceRoot(cwd);
  const reconciledJobs = listReconciledJobs(workspaceRoot);
